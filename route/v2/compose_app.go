...
import (
	...
	"github.com/Vioneta/VionetaOS-AppManagement/service"
...
func (a *AppManagement) ApplyComposeAppSettings(ctx echo.Context, id codegen.ComposeAppID, params codegen.ApplyComposeAppSettingsParams) error {
	...
	if uncontrolled {
		...
		xvionetaos := composeApp.Extensions[common.ComposeExtensionNameXVionetaOS]
		xvionetaosMap, ok := xvionetaos.(map[string]interface{})
		if !ok {
			logger.Error("failed to get map compose app extensions", zap.String("composeAppID", composeApp.Name))
		} else {
			xvionetaosMap[common.ComposeExtensionPropertyNameIsUncontrolled] = true
			composeApp.Extensions[common.ComposeExtensionNameXVionetaOS] = xvionetaosMap
		...
		if !ok {
			...
			xvionetaosMap[common.ComposeExtensionPropertyNameIsUncontrolled] = false
			composeApp.Extensions[common.ComposeExtensionNameXVionetaOS] = xvionetaosMap
...
func (a *AppManagement) InstallComposeApp(ctx echo.Context, params codegen.InstallComposeAppParams) error {
	...
	if err := service.MyService.Compose().Install(backgroundCtx, composeApp); err != nil {
		...
		message := err.Error()
		if err == service.ErrComposeExtensionNameXVionetaOSNotFound {
...
func composeAppsWithStoreInfo(ctx context.Context) (map[string]codegen.ComposeAppWithStoreInfo, error) {
	...
	return lo.MapValues(composeApps, func(composeApp *service.ComposeApp, id string) codegen.ComposeAppWithStoreInfo {
		...
		isUncontrolled, ok := composeApp.Extensions[common.ComposeExtensionNameXVionetaOS].(map[string]interface{})[common.ComposeExtensionPropertyNameIsUncontrolled].(bool)
		if ok {

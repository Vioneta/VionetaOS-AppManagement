...
var replaceUrl = []UrlReplacement{
	...
	},
	{
		...
		NewUrl: "https://vionetaos.oss-cn-shanghai.aliyuncs.com/store/main.zip",
	},
}
...
func (u *migrationTool044AndOlder) IsMigrationNeeded() (bool, error) {
	...
	for _, v := range replaceUrl {
		if strings.Contains(string(content), v.OldUrl) {
			_logger.Info("Migration is needed for a VionetaOS with old app store link.")
			return true, nil
		}
	}
	return false, nil
...
func (u *migrationTool044AndOlder) Migrate() error {
	...
	// replace https://github.com/Vioneta/_appstore/archive/refs/heads/main.zip to https://vionetaos-appstore.github.io/vionetaos-appstore/linux-all-appstore.zip

...
import (
	...
	v1 "github.com/Vioneta/VionetaOS-AppManagement/service/v1"
	"github.com/bluele/gcache"

	"github.com/Vioneta/VionetaOS-AppManagement/codegen"
	"github.com/Vioneta/VionetaOS-AppManagement/common"
	"github.com/Vioneta/VionetaOS-AppManagement/pkg/config"
	...
	"github.com/Vioneta/VionetaOS-Common/utils/random"
...
func (a *ComposeApp) StoreInfo(includeApps bool) (*codegen.ComposeAppStoreInfo, error) {
	...
	if !ok {
		return nil, ErrComposeExtensionNameXVionetaOSNotFound
	}
	...
	if includeApps {
		...
		for _, app := range a.Apps() {
			...
			if err != nil {
				if err == ErrComposeExtensionNameXVionetaOSNotFound {
					logger.Info("App does not have x-vionetaos extension - skipping", zap.String("app", app.Name))
					continue
				}
...
func (a *ComposeApp) SetStoreAppID(storeAppID string) (string, bool) {
	...
	extension, ok := a.Extensions[common.ComposeExtensionNameXVionetaOS]
	if !ok {
		logger.Info("compose app does not have x-vionetaos extension - might not be a compose app for VionetaOS", zap.String("app", a.Name))
		return "", false
	}

	composeAppStoreInfo, ok := extension.(map[string]interface{})
	if !ok {
		logger.Info("compose app does not have valid x-vionetaos extension - might not be a compose app for VionetaOS", zap.String("app", a.Name))
		return "", false
	}
...
func (a *ComposeApp) SetTitle(title, lang string) {
	...
	if !ok {
		...
		a.Extensions[common.ComposeExtensionNameXVionetaOS] = extension
	}
	...
	if !ok {
		logger.Info("compose app does not have valid title map in its x-vionetaos extension - might not be a compose app for VionetaOS", zap.String("app", a.Name))
		return
	}
...
func NewComposeAppFromYAML(yaml []byte, skipInterpolation, skipValidation bool) (*ComposeApp, error) {
	tmpWorkingDir, err := os.MkdirTemp("", "vionetaos-compose-app-*")
	if err != nil {
		return nil, err
	}

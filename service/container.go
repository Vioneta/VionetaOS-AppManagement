...
import (
	...
	"github.com/Vioneta/VionetaOS-AppManagement/model"
	"github.com/Vioneta/VionetaOS-AppManagement/pkg/config"
	"github.com/Vioneta/VionetaOS-AppManagement/pkg/docker"
	"github.com/Vioneta/VionetaOS-AppManagement/pkg/utils/envHelper"
	...
	timeutils "github.com/Vioneta/VionetaOS-Common/utils/time"
...
func getContainerStats() {
	...
	for i := 0; i < 100; i++ {
		...
		if config.VionetaOSGlobalVariables.AppChange {
			config.VionetaOSGlobalVariables.AppChange = false
...
func (ds *dockerService) GetContainerAppList(name, image, state *string) (*[]model.MyAppList, *[]model.MyAppList) {
	...
	defer cli.Close()
	// fts := filters.NewArgs()
	// fts.Add("label", "vionetaos=vionetaos")
	// fts.Add("label", "vionetaos")
	// fts.Add("vionetaos", "vionetaos")
	containers, err := cli.ContainerList(context.Background(), types.ContainerListOptions{All: true})
	if err != nil {
		logger.Error("Failed to get container_list", zap.Any("err", err))
	}
	...
	for i, m := range containers {
		...
		if m.Labels["vionetaos"] == "vionetaos" {
			...
			if m.Labels["origin"] == "system" {
				...
				if len(strings.Split(name, "/")) > 1 {
					icon = "https://icon.vionetaos.io/main/all/" + strings.Split(name, "/")[1] + ".png"
				}
			}
...
func (ds *dockerService) CreateContainer(m model.CustomizationPostData, id string) (containerID string, err error) {
	...
	if err == nil {
		...
		config = info.Config
		if config.Labels["vionetaos"] == "vionetaos" {
			config.Cmd = m.Cmd
	...
	config.Labels["vionetaos"] = "vionetaos"
